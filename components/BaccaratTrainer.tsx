...
const TABLE_MIN = 200000;
const TABLE_MAX = 30000000;
const MAN = 10000;

// 파라다이스 실사양 칩 6종
const CHIPS = [
  { v: 1000000, name: "금", c: "#d4af37", edge: "#8a6d1c", tc: "#3a2f0a" },
  { v: 100000, name: "녹", c: "#2e7d4f", edge: "#1a4a2e", tc: "#eaf6ee" },
  { v: 10000, name: "적", c: "#c0392b", edge: "#7e2218", tc: "#fdeeee" },
  { v: 5000, name: "청", c: "#2e6db4", edge: "#1b4470", tc: "#e9f2fc" },
  { v: 1000, name: "백", c: "#f0ede2", edge: "#b9b4a4", tc: "#4a4942" },
  { v: 500, name: "회", c: "#9a9a96", edge: "#5f5f5c", tc: "#26261f" },
];
const BETTABLE = [1000000, 100000, 10000]; // 베팅 가능 칩
const SMALL = [5000, 1000, 500]; // 커미션/체인지 전용

// 씬 좌표계 (380 × 300)
const SW = 380, SH = 300;
const ARM_ORIGIN = { x: 225, y: 332 };
const POS_SHOE = { x: 46, y: 232 };
const POS_BZONE = { x: 122, y: 190 };
const POS_PZONE = { x: 258, y: 190 };
const POS_REST = { x: 225, y: 300 };
const POS_TRAY = { x: 190, y: 296 };

const SUITS = [
  { s: "♠", c: "#221d18" }, { s: "♥", c: "#b3372f" },
  { s: "♦", c: "#b3372f" }, { s: "♣", c: "#221d18" },
];
...
const handTotal = (cards) => cards.reduce((s, c) => s + cardValue(c), 0) % 10;
function bankerShouldDraw(bT, pThird) {
...
function chipBreakdown(amount, denoms) {
...
const minimalChipCount = (amount) => chipBreakdown(amount).reduce((s, g) => s + g.cnt, 0);
const groupsTotal = (groups) => groups.reduce((s, g) => s + g.denom * g.count, 0);
const chipMeta = (v) => CHIPS.find((c) => c.v === v);
// 존(zone) 그룹 병합/제거 — denom당 하나의 그룹 유지
function mergeAdd(groups, denom, nAdd) {
...
function mergeRemove(groups, denom, nSub) {
  return groups.map((g) => (g.denom === denom ? { ...g, count: g.count - nSub } : g)).filter((g) => g.count > 0);
}
// 컷팅 스프레드 패턴 — 20 초과: 20단위 뭉탱이 분리 / 나머지: 1~5 전부 스프레드, 6=3+3, 7=3+3+1, 8=4+4, 9=4+4+1, 10~20 = 5컷 + 자투리
// 자투리 그룹이 있으면 마지막 두 그룹 스프레드(컷 증명 + 자투리), 없으면 마지막 한 그룹만 스프레드
function spreadPattern(count) {
  if (count <= 0) return [];
  const bigs = [];
  let r = count;
...
function seatX(i, n) { return n === 1 ? 190 : 36 + 308 * (i / (n - 1)); }
function seatY(i, n) { const t = n === 1 ? 0.5 : i / (n - 1); return 16 + 20 * Math.pow(2 * t - 1, 2); }
function spotPos(i, n) { const sx = seatX(i, n); return { x: 190 + (sx - 190) * 0.8, y: seatY(i, n) + 50 }; }

// ═══════════ 사운드 (Web Audio 합성 — 음원 파일 불필요) ═══════════
let _ac = null;
let _sfxOn = true;
function setSfxOn(v) { _sfxOn = v; }
function ac() {
  if (!_ac) {
...
// ═══════════ 손님 페르소나 (베팅: 만원 단위, MIN 20만) ═══════════
const PERSONAS = [
  { id: "park", name: "박 상무", emoji: "🧐", tag: "VIP · 뱅커 단골",
    desc: "50대 VIP 사업가. 뱅커에만 크게 베팅하며 커미션 계산에 매우 민감하다. 격식을 차리지만 까다롭고, 지급액이 조금이라도 이상하면 바로 지적한다.",
  ...
  { id: "wang", name: "왕웨이", emoji: "😄", tag: "중국 단골 · 타이 애호",
    desc: "중국에서 온 단골. 한국어가 서툴러 중국어를 섞어 쓴다 (운이 나쁘면 '运气不好', 좋으면 '太好了!'). 타이 베팅을 좋아하고 미신을 중시한다. 딜러가 중국어로 한마디 해주면 매우 좋아한다.",
  ...
  { id: "mina", name: "미나", emoji: "🌸", tag: "첫 방문 · 신혼여행",
    desc: "신혼여행으로 제주에 온 20대 후반. 카지노가 처음이라 룰을 모르고 질문이 많다. 친절히 설명하면 고마워하지만 무시당하면 위축된다.",
  ...
  { id: "jung", name: "정 사장", emoji: "🍷", tag: "유쾌한 단골 · 한잔 걸침",
    desc: "와인을 몇 잔 걸친 유쾌한 60대 단골. 농담을 즐기고 따면 딜러에게 팁을 주려 한다. 가끔 테이블 위 카드를 만지려 한다 (규정 위반).",
  ...
  { id: "tanaka", name: "다나카", emoji: "🤓", tag: "일본 · 출목표 분석가",
    desc: "정중한 60대 일본인. 한국어에 일본어가 섞인다 ('스미마셍', '소데스네'). 출목표에 매 판을 기록하며 진행이 빠르면 곤란해하고 직전 카드 순서를 다시 묻는다.",
  ...
  { id: "mike", name: "Mike", emoji: "🤠", tag: "미국 관광객 · 팁 후함",
    desc: "하와이안 셔츠의 30대 미국인. 영어 반 한국어 반, 목소리가 크다. 따면 하이파이브를 요구하고 딜러 몫 팁 벳을 올리려 한다. 악의는 없지만 시끄럽다.",
    betSide: () => (Math.random() < 0.7 ? "player" : "banker"),
  ...
  { id: "okja", name: "옥자 이모", emoji: "💄", tag: "단골 · 딜러 바라기",
    desc: "50대 단골 아주머니. 딜러가 마음에 들어 '복덩이'라 부르고 커피를 사주겠다거나 나이·연애 같은 사적인 질문을 한다. 악의는 없지만 게임을 늦춘다. 친근함을 잃지 않으면서 부드럽게 선을 긋는 훈련 상대.",
  ...
  { id: "kang", name: "강 실장", emoji: "😏", tag: "⚠ 선 넘는 손님",
    desc: "술기운 오른 40대 임원. 외모 평가, 퇴근 시간·연락처 같은 부적절한 사적 질문을 하고 지면 딜러 탓에 반말을 한다. 정중하지만 단호한 경계 긋기, 반복 시 핏 보스 호출 판단을 훈련하는 캐릭터.",
  ...
  { id: "lyudmila", name: "류드밀라", emoji: "🧊", tag: "러시아 · 하이롤러",
    desc: "말이 거의 없는 40대 러시아인 하이롤러. 무표정하게 큰 금액을 베팅하고 절차에 흔들림이 보이면 짧고 차갑게 지적한다. 과한 친절보다 정확하고 군더더기 없는 진행을 원한다.",
  ...
  { id: "choi", name: "최 영감", emoji: "👴", tag: "30년 단골 · 이야기꾼",
    desc: "30년 단골 70대 할아버지. 느긋하고 정이 많지만 한번 입을 열면 88년도 옛날이야기가 길어져 게임이 지연된다. 어르신을 존중하면서 자연스럽게 게임을 진행시키는 요령이 필요하다.",
    betSide: () => (Math.random() < 0.5 ? "player" : "banker"),
    betAmt: () => [20, 30, 50][Math.floor(Math.random() * 3)] * MAN,
...
function makeViolation(cust) {
  const pool = [
    ...
    { type: "invalidChip", text: `베팅 불가 칩(청·백)을 스팟에 올림`, applyBet: null, fixBet: null, invalidChip: true,
      ...
      fixNote: "5천·1천·5백 칩은 커미션 거스름 전용으로, 베팅 스팟에 올릴 수 없습니다. 정중히 안내하고 베팅 가능 칩으로 교환해 드립니다." },
    { type: "cash", text: `칩이 아닌 현금을 레이아웃 위에 올림`, applyBet: null, fixBet: null, cash: true,
      correct: `"현금은 칩으로 교환해 드리겠습니다" — 체인지 콜`,
      fixNote: "레이아웃 위 현금은 체인지 콜 후 칩으로 교환하는 것이 원칙입니다. 현금이 올라간 채 게임을 진행하면 안 됩니다." },
  ];
  return { cust, ...pool[Math.floor(Math.random() * pool.length)], resolved: false };
}
// 위반 발생 시 손님의 중립적 잡담 (위반을 누설하지 않음)
const NEUTRAL_LINES = ["이번 판 느낌이 좋은데…", "(칩을 만지작거린다)", "자, 가 봅시다.", "흠… 이쯤이면 됐나.", "(테이블을 톡톡 두드린다)"];

// ═══════════ 표현 컴포넌트 (모듈 레벨 — 리마운트 방지) ═══════════
const Chip = ({ meta, w, h }) => (
...
// 풀 스택 (워크벤치용) — 5개 단위 틈, 20개 초과 분할
const ChipStackVisual = ({ amount, chipW = 30, showAmount, maxPerCol = 20 }) => {
...
// 컷팅 스프레드 뷰 — denom 하나의 칩 더미를 표준 패턴으로 렌더, 그룹 탭 가능
const SpreadGroups = ({ denom, count, chipW = 22, onTapGroup, dim }) => {
...
// 씬용 미니 스택
const MiniStack = ({ amount, invalid }) => {
  const groups = chipBreakdown(amount);
  const cols = [];
  groups.forEach((g) => {
    let left = g.cnt;
...
const CardView = ({ card, w = 30, sideways }) => {
  const h = w * 1.45;
  const inner = (
...
const ActionBtn = ({ children, onClick, accent, disabled }) => (
...
const thtd = { border: "1px solid rgba(246,241,227,.14)", padding: "4px 8px", textAlign: "center" };
const sideLabel = { player: "PLAYER", banker: "BANKER", tie: "TIE" };
const sideColor = { player: BLUE, banker: RED, tie: GOLD };
const dealBtnStyle = (color) => ({ flex: 1, background: "rgba(246,241,227,.05)", border: `1.5px solid ${color}99`, color: IVORY, borderRadius: 10, padding: "14px 8px", fontSize: 13.5, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" });
    ...
    (async () => {
      try {
        if (typeof window !== "undefined" && window.storage) {
          const r = await window.storage.get("baccarat-best");
...
다른 텍스트 없이 JSON만: {"pass":false,"coaching":"..."}`);
  ...
  async function saveAndExit() {
    ...
    if (!prev || score > prev.score) {
      ...
      setRecords(next);
      try {
        if (typeof window !== "undefined" && window.storage) await window.storage.set("baccarat-best", JSON.stringify(next));
      } catch (e) { /* 저장 실패해도 진행 */ }
    }
