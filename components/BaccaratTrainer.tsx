...
export default function BaccaratDealerTrainerV3() {
  ...
  useEffect(() => {
    if (phase === "betting") {
      showGuide("betting", "칩 색상 & 베팅 감시",
        "먼저 칩 색상을 기억해 두세요.\n🟡 금색 100만 · 🟢 녹색 10만 · 🔴 적색 1만\n🔵 청색 5천 · ⚪ 백색 1천 · ⬜ 회색 500원\n\n" +
        (mode === "practice"
          ? "손님들이 베팅 중입니다. 금액과 위치를 확인하세요.\n• 한도 초과·현금 사용·규정 외 칩 → 규정 위반\n• 손님 이름 탭 → 규정 위반 지적 버튼"
          : "실전 모드 — 칩 색과 개수로 금액을 읽어야 합니다.\n• 손님 이름 탭 → 규정 위반 지적") +
        "\n\n💡 화면 하단: 📊 칩 차트 · 📋 3rd 룰 · 📖 용어사전\n우상단 ✕ → 저장하고 나가기");
      ...
      showGuide("dealing", "딜링 순서",
        "카드를 슈에서 꺼내 딜링합니다.\n\n• 순서는 반드시 P → B → P → B (플레이어 먼저)\n• 아래 버튼으로 딜링할 대상을 선택하세요.\n• 순서를 틀려도 카드는 자동으로 올바른 위치에 놓입니다. 단, 감점됩니다.\n• 3rd 카드 룰 차트는 하단 📋 버튼에서 확인할 수 있습니다.");
    } else if (phase === "openPlayerCards" || phase === "callInitial") {
      showGuide("callInitial", "초기 핸드 콜 & 내추럴", "카드 4장이 모두 배분되었습니다.\n\n• 「카드 오픈」 후 플레이어와 뱅커의 합을 확인하세요.\n• 어느 쪽이든 합이 8 또는 9면 내추럴(Natural) — 추가 딜링 없이 바로 결과를 선언합니다.\n• 콜 옵션에서 합계를 정확히 선택하세요.");
    } else if (phase === "playerThird" || phase === "openPlayerThird") {
      showGuide("playerThird", "플레이어 3rd 카드 룰", "플레이어 합이 0~5이면 한 장 더 받습니다.\n합이 6~7이면 스탠드(추가 없음).\n\n• 내추럴(8·9)은 이미 처리되었습니다.\n• 「Player에게 3rd」 또는 「Player 스탠드」를 선택하세요.\n• 헷갈리면 하단 📋 3rd 카드 룰 차트를 확인하세요.");
    } else if (phase === "bankerThird" || phase === "openBankerThird") {
      showGuide("bankerThird", "뱅커 3rd 카드 룰", "뱅커의 드로우는 플레이어 3rd 카드 값에 따라 결정됩니다.\n\n• 플레이어 스탠드 시: 뱅커 합 0~5 → 드로우, 6~7 → 스탠드\n• 플레이어가 3rd를 받은 경우: 하단 📋 룰 차트를 참고하세요.\n• 드로우/스탠드 버튼으로 선택하세요.");
      ...
      showGuide("settleOrder", mode === "practice" ? "정산 순서 (연습)" : "정산 순서 (실전)", mode === "practice"
        ? "승패가 결정되면 정산 순서에 따라 버튼을 누릅니다.\n\n• ① 진 쪽 칩을 테이크(수거)\n• ② 이긴 쪽에 페이(지불)\n• BANKER 승리 시 배당에서 커미션 5%를 공제한 금액을 지급합니다.\n\n예) 손님이 100만원 베팅 → 95만원 지급, 5만원은 커미션으로 카지노 수익"
        : "실전 모드 정산입니다.\n\n• ① 진 쪽 칩 수거 → ② 이긴 쪽 페이\n• BANKER 승리 시: 배당액을 컷팅 후 커미션(5%) 공제액을 별도 수거합니다.\n• 커미션은 'W/B 커미션' 항목에서 처리하며, 라운드 종료 후 손님이 직접 납부합니다.\n• 칩을 직접 구성해야 하므로 금액을 정확히 계산하세요.");
    } else if (phase === "takeWB") {
      showGuide("takeWB", "실전 — 테이크 & 커미션 수거",
